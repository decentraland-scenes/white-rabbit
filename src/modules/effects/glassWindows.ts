...
export class GlassController {
   ...
   glassWindowsBroken:Entity[] 
    ...
    addGlassPlanes(center:Vector3, radius:number){  
        ...
        glassBrokenDummy.addComponent(new Transform({
        ...
        for(let i=0; i < 16; i++){
            ...
            glassBroken.addComponent(new OnPointerDown( (e)=>{
                        this.shatterGlass(i)
                    },
    ...
    rebuildGlasses(glass:Entity){
        ...
        for(let i = 0; i< this.glassWindows.length; i++){
            this.glassWindows[i].getComponent(Transform).position.y = scene.venueCenter.y +2.35
        }
    }
    ...
    shatterGlass(_index:number){
        ...
        this.glassWindowsBroken[_index].getComponent(ShatterGlass).isShattered = true
        this.glassWindowsBroken[_index].getComponent(Transform).position.y = scene.venueCenter.y +2.35
