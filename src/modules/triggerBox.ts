...
export class TriggerBox extends Entity
{
  ...
  constructor(_pos:Vector3, _size:Vector3){
    ...
    this.addComponent(new Transform({position:_pos,
  ...
  collide(pos:Vector3):boolean{
    ...
    if(pos.x >  this.areaXMin && pos.x <  this.areaXMax && pos.y >  this.areaYMin && pos.y <this.areaYMax && pos.z > this.areaZMin && pos.z < this.areaZMax ){
      if(!this.fired){
